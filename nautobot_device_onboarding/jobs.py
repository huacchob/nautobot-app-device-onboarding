...
class SSOTSyncDevices(DataSource):  # pylint: disable=too-many-instance-attributes
    ...
    def run(  # pylint: disable=too-many-positional-arguments
        ...
        if csv_file:
            ...
            if platform and not self._validate_platform_network_driver(platform):
...
class SSOTSyncNetworkData(DataSource):  # pylint: disable=too-many-instance-attributes
    ...
    sync_modules = BooleanVar(
        default=False, description="Sync modules (linecards, supervisors, fan trays, power supplies) from device."
    )
    namespace = ObjectVar(
        model=Namespace,
        required=True,
